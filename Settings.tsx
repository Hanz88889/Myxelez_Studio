...
import {
  ...
  KeyRound, Link2, Plus, RefreshCw, Shield, Smartphone, Trash2, User, Wallet, Zap,
} from 'lucide-react'
import { API_KEYS_SEED, CONNECTED, CREDIT_HISTORY, INVOICES, PLANS, SESSIONS, USAGE_DAYS, USAGE_WEEK } from '../lib/data'
import { cn, copyText, formatIDR, uid } from '../lib/utils'
...
const TABS = [
  ...
  { id: 'topup', label: 'Top Up', icon: <Zap size={13} /> },
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center gap-5">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2.5 flex-wrap">
              <h3 className="font-display text-xl font-semibold">2,880 CR</h3>
              <Badge variant="mint">Pay-as-you-go</Badge>
            </div>
            <p className="text-[13px] text-smoke mt-1">Saldo kredit kamu — tidak ada tagihan bulanan, tidak direset, tidak kedaluwarsa.</p>
          </div>
          <Button variant="chrome" onClick={() => toast('Scroll ke paket top-up di bawah', 'info')}>Top up lagi</Button>
        </div>
      </Card>
      <div className="grid md:grid-cols-3 gap-4">
        {PLANS.map((p) => (
          <Card key={p.id} hover className={cn('p-5', p.featured && 'border-edge bg-glass2')}>
            <div className="flex items-center justify-between">
              <h4 className="font-display font-semibold">{p.name}</h4>
              {p.featured && <Badge variant="chrome">Populer</Badge>}
            </div>
            <p className="font-display text-2xl font-medium mt-2 text-chrome">{formatIDR(p.price)}</p>
            <p className="text-[12px] text-smoke mt-0.5">
              ...
              {p.bonus > 0 && <span className="text-mint"> (+{p.bonus.toLocaleString('id-ID')} bonus)</span>}
            </p>
            ...
            <Button variant={p.featured ? 'chrome' : 'outline'} size="sm" className="w-full mt-5" onClick={() => toast(`Top up ${p.name} — mengarahkan ke pembayaran`)}>
              Beli sekarang
            </Button>
          </Card>
        ))}
    ...
    <div className="grid lg:grid-cols-[340px_1fr] gap-5">
      <Card spotlight={false} className="p-6">
        ...
        <div className="mt-5 pt-5 border-t border-line flex flex-col gap-2.5 text-[12.5px]">
          ...
          <div className="flex justify-between"><span className="text-ash">Top up terakhir</span><span className="text-mist">11 Feb 2026</span></div>
          <div className="flex justify-between"><span className="text-ash">Auto-charge</span><span className="text-mist">Tidak aktif — bayar per top up</span></div>
        </div>
      </Card>
      <Card spotlight={false} className="overflow-hidden">
        <div className="px-6 py-4 border-b border-line flex items-center justify-between">
          <h3 className="font-display text-[15px] font-semibold">Riwayat Top Up</h3>
