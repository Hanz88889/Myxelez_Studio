          ...
          <AnimatePresence initial={false}>
            {open === i && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.32, ease: [0.22, 1, 0.36, 1] }}>
                <p className="px-5 sm:px-6 pb-5 text-[13.5px] text-smoke leading-relaxed">{it.a}</p>
              </motion.div>
            )}
          </AnimatePresence>
        </Card>
...
/* ─── SectionHead ─── */
export function SectionHead({ center, kicker, title, desc }: { center?: boolean; kicker: string; title: ReactNode; desc?: string }) {
  return (
    <div className={cn('max-w-2xl', center && 'mx-auto text-center')}>
      <Reveal><Badge variant="chrome">{kicker}</Badge></Reveal>
      <Reveal delay={0.08}>
        <h2 className="mt-4 font-display text-3xl sm:text-5xl font-medium tracking-tight leading-[1.05]">{title}</h2>
      </Reveal>
      {desc && (
        <Reveal delay={0.16}>
          <p className="mt-4 text-smoke text-[15px] leading-relaxed">{desc}</p>
        </Reveal>
      )}
    </div>
  )
...
export function EmptyState({ icon, title, desc, action }: { icon?: ReactNode; title: string; desc?: string; action?: ReactNode }) {
  return (
    <div className="flex flex-col items-center text-center py-16 px-6">
      <span className="h-14 w-14 rounded-2xl glass flex items-center justify-center text-ash mb-4">{icon}</span>
      <p className="font-display text-lg font-semibold text-mist">{title}</p>
      {desc && <p className="text-[13px] text-ash mt-1.5 max-w-xs leading-relaxed">{desc}</p>}
...
/* ─── Toast system ─── */
interface ToastItem { id: string; message: string; variant: 'ok' | 'err' | 'info' }
const ToastContext = createContext<(message: string, variant?: ToastItem['variant']) => void>(() => {})

export function ToastProvider({ children }: { children: ReactNode }) {
  const [toasts, setToasts] = useState<ToastItem[]>([])

  const toast = useCallback((message: string, variant: ToastItem['variant'] = 'ok') => {
        ...
        <AnimatePresence>
          {toasts.map((t) => (
            <motion.div
              ...
              className="glass-2 rounded-xl pl-3 pr-4 py-3 text-[13px] font-medium shadow-2xl max-w-xs flex items-center gap-2.5 pointer-events-auto"
            >
              <span className={cn(
                'h-6 w-6 rounded-full flex items-center justify-center shrink-0',
                t.variant === 'ok' && 'bg-mint/15 text-mint',
                t.variant === 'err' && 'bg-blush/15 text-blush',
                t.variant === 'info' && 'bg-frost/15 text-frost',
              )}>
                {t.variant === 'ok' && <Check size={13} />}
                {t.variant === 'err' && <X size={13} />}
                {t.variant === 'info' && <Info size={13} />}
              </span>
              <span className="text-mist">{t.message}</span>
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
