...
import { Badge, Button, Logo, Menu, Tooltip, useToast } from './ui'
...
function CommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const commands = [
    ...
    { label: 'Billing & Top Up', icon: CreditCard, run: () => navigate('/studio/settings?tab=billing') },
        ...
        {!(collapsed && !mobile) ? (
          <div className="glass rounded-2xl p-4 mb-3 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3.5">
              <span className="h-11 w-11 rounded-xl btn-chrome flex items-center justify-center shrink-0">
                <Zap size={18} fill="currentColor" />
              </span>
              <div className="min-w-0">
                <p className="text-[13px] font-semibold text-mist">2,880 CR</p>
                <p className="text-[11px] text-ash">Pay-as-you-go · no expiry</p>
              </div>
            </div>
            <Button variant="chrome" size="sm" className="w-full mt-3.5" onClick={() => navigate('/studio/settings?tab=topup')}>
              <Zap size={13} /> Top up
            </Button>
          </div>
        ) : (
          <Tooltip label="2,880 CR" side="right">
            <div className="flex justify-center mb-3">
              <span className="h-9 w-9 rounded-lg btn-chrome flex items-center justify-center">
