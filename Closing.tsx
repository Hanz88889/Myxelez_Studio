    ...
    <section id="pricing" className="relative py-24 sm:py-32 border-t border-line">
      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6">
        <SectionHead
          center
          kicker="Pricing"
          title={<>Simple plans, <span className="text-chrome">serious output.</span></>}
          desc="Start free. Upgrade when the work demands it. Every plan includes all creation tools — you only scale engines, resolution and seats."
        />
        <Reveal className="mt-9 flex items-center justify-center gap-3">
          <span className={cn('text-sm', !yearly ? 'text-mist' : 'text-ash')}>Monthly</span>
          <button
            type="button"
            onClick={() => setYearly((y) => !y)}
            className={cn('relative h-7 w-13 rounded-full border transition-colors cursor-pointer', yearly ? 'bg-gradient-to-b from-white/90 to-zinc-400 border-white/40' : 'bg-haze border-line')}
            aria-label="Toggle yearly billing"
          >
            <span className={cn('absolute top-1/2 -translate-y-1/2 h-5 w-5 rounded-full shadow transition-all duration-300', yearly ? 'right-1 bg-void' : 'left-1 bg-ash')} />
          </button>
          <span className={cn('text-sm', yearly ? 'text-mist' : 'text-ash')}>Yearly</span>
          <Badge variant="mint">Save 20%</Badge>
        </Reveal>
            ...
            <Reveal key={p.id} delay={i * 0.09} className="h-full">
              ...
              >
                ...
                <div className="mt-5 flex items-end gap-1.5">
                  <span className="font-display text-[2.6rem] leading-none font-medium text-chrome">${yearly ? p.yearly : p.monthly}</span>
                  <span className="text-[12px] text-ash mb-1.5">/ month{yearly && p.monthly > 0 ? ' · billed yearly' : ''}</span>
                </div>
        ...
        <Reveal className="mt-8 text-center">
          <p className="text-[12px] text-ash">All prices in USD. Cancel anytime — credits remain valid until the cycle ends.</p>
        </Reveal>
      </div>
    </section>
  )
          ...
          <div className="relative overflow-hidden rounded-[2rem] border border-edge">
            ...
            <div className="relative px-6 py-20 sm:py-28 text-center">
              ...
              <div className="mt-9 flex flex-col sm:flex-row items-center justify-center gap-3">
                <Link to="/studio"><Button variant="chrome" size="lg">Launch the Studio <ArrowRight size={15} /></Button></Link>
